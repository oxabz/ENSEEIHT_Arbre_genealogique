...
Pour récupérer le descendant d'un individue il va nous falloir trouver l'individue. On pourrait utiliser ``rechercher`` mais nous allons avoir besoin de garder le chemin emprunté pour trouver l'individue. On fait donc une recherche récursive. Une fois l'individu trouvé on fait redescendre sa profondeur. Si la profondeur du descendant rempli le critere``profndeur_cible + profondeur_noeud  = profondeur_ancetre``  on fait re
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzIxMDU4NDAwLDEzMzY2OTAwMCwtMTM3OT
